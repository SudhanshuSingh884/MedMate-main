...
import 'package:medmate/views/profile_page.dart';
import 'package:sqflite/sqflite.dart';
import 'package:path/path.dart';
import 'package:web_socket_channel/io.dart';
import 'package:medmate/views/clock_view.dart';
import 'package:medmate/views/previous_alarms.dart';
...
class _WebSocketDHT extends State<WebSocketDHT> {
  ...
  Future<List<String>> readdata() async {
    ...
    for (int i = 0; i < lastalarm.length; i++) {
      var ttitt = lastalarm[i]["cabin"].toString() +
          ...
          "_" +
          lastalarm[i]["id"].toString() +
          "_" +
          lastalarm[i]["title"].toString();
  ...
  Future<void> _addData(
      String cabinss, String taken, String titles, String datesofalarms) async {
    _database2 = await openDatabase(
      join(await getDatabasesPath(), 'my_database.db'),
      onCreate: (db, version) {
        db.execute(
    ...
    await _database2.insert('my_table', {
      'cabinss': cabinss,
      'taken': taken,
      "titles": titles,
      "datesofalarms": datesofalarms
    });
    _refreshData();
  }
          ...
          Expanded(
            child: Consumer<MenuInfo>(
              builder: (BuildContext context, MenuInfo value, Widget? child) {
                if (value.menuType == MenuType.AddPill) return AlarmPage();

                if (value.menuType == MenuType.PreviousPills)
                  return TablePage();

                if (value.menuType == MenuType.Profile) return ProfilePage();
                ...
                return Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      // Text(
                      //   connected
                      //       ? "Device connected to Arduino"
                      //       : "Arduino is disconnected. Check WiFi or prees the button ",
                      //   style: TextStyle(
                      //     fontFamily: "avenir",
    ...
    return Consumer<MenuInfo>(
      builder: (BuildContext context, MenuInfo value, Widget? child) {
        return TextButton(
          ...
          child: Column(
            children: <Widget>[
              Image.asset(currentMneuInfo.imageSource!, scale: 1.5),
              SizedBox(height: 16),
